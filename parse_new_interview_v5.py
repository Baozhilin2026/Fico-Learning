...
def parse_interview_docx(file_path):
    ...
    while i < len(doc.paragraphs):
        ...
        # 检测英文答案的开始
        if 'Answer 英文' in text and current_question and not current_question['sampleAnswer']:
            # 移动到下一行开始读取
            i += 1
            answer_lines = []
        ...
        # 检测中文答案的开始
        if 'Answer 中文' in text and current_question and not current_question['sampleAnswerCN']:
            # 移动到下一行开始读取
            i += 1
            answer_lines = []
            ...
            while i < len(doc.paragraphs):
                ...
                # 遇到下一个英文答案标记就停止（防止包含下一题）
                if 'Answer 英文' in ans_text:
                    break
    ...
    for part in parts:
        for q in part['questions']:
            if q['questionNo'] in ['Q34', 'Q41', 'Q56', 'Q57']:
                ...
                print(f"{q['questionNo']}: EN={en_len} chars, CN={cn_len} chars")
                if en_len > 0:
                    print(f"  EN preview: {q['sampleAnswer'][:100]}...")
                if cn_len > 0:
                    print(f"  CN preview: {q['sampleAnswerCN'][:100]}...")
